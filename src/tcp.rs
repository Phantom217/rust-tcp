...
/// ```
///
/// [(RFC 793 S3.2 F4)]: https://tools.ietf.org/html/rfc793#section-3.2
...
/// ```
///
/// [(RFC 793 S3.2 F5)]:  https://tools.ietf.org/html/rfc793#section-3.2
...
impl Connection {
    ...
    pub(crate) fn on_tick(&mut self, nic: &mut tun_tap::Iface) -> io::Result<()> {
        if let State::FinWait2 | State::TimeWait = self.state {
            // we have shutdown our write side and the other side acked, no need to (re)transmit
            // anything
            return Ok(());
        }
